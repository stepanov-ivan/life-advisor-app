...
- Реализовать 7 вычислительных примитивов: `range`, `max`, `min`, `maxWeekly`, `presence`, `countSkipped`, `frequency`.
- Правила оцениваются в скользящем 7-дневном окне с минимумом 3 днями данных (≥1 structured приём в день).
- Расчёт инкрементальный: первичный слепок на 7 дней, далее только дельта нового дня.
- Статус правила отражает сегодняшний день (daily freshness), а не всё окно.
- Три зоны оценки: normal / warning (≥85% порога) / violation (превышение порога).
- Все правила включены по умолчанию. Пользователь может включить/выключить каждое правило.
...
- Экран детали правила с графиком за 7 дней и списком нарушений с drill-down: день → приём → ингредиент.
- Навигация от нарушения: переключение таба на Дашборд + установка даты + открытие приёма.
- Подсветка нарушений в ленте питания: цветной левый бордер (жёлтый/красный) и бейдж с количеством нарушений у MealSlotCard; иконка у ингредиента-нарушителя в MealEventEditorView.
- Контекст нарушения: какое правило, причина, величина отклонения, ссылка на приём пищи.

## 16 правил ВОЗ (4 категории)

### Баланс макронутриентов
1. Белок 10–15% энергии (range)
2. Жиры 15–30% энергии (range)
3. Насыщенные жиры ≤10% энергии (max)
4. Трансжиры ≤1% энергии (max)
5. ПНЖК 6–10% энергии (range)
6. Углеводы 45–75% энергии (range)

### Ограничения
7. Свободный сахар ≤10% энергии (max)
8. Соль ≤2000 мг натрия (max)
9. Энергобаланс: превышение >15% = warning, >25% = violation (frequency: 1–2 дня = warning, 3+ = violation из 7)
10. Клетчатка ≥25 г (min)

### Качество продуктов
11. Овощи и фрукты ≥400 г/день (min, сумма grams по категориям fruit + vegetable)
...
14. Красное мясо ≤500 г/неделю (maxWeekly, сумма grams по foodCategory=red_meat)
15. Обработанное мясо отсутствует (presence, ни одного с foodCategory=processed_meat)

### Режим питания
16. Регулярность приёмов: пропуски окон (countSkipped, 1–3 = warning, 4+ = violation из 7)
...
- `NutritionRuleConfig`: ruleId (String), isEnabled (Bool). Создаётся для каждого правила при первом запуске.
- `RuleViolation`: ruleId, date, zone ("warning" | "violation"), magnitude, reasonCode, mealEventId? (UUID), estimateItemId? (UUID).
...
- `Services/RuleEngine.swift` — загрузка JSON из bundle, evaluation orchestration, инкрементальный пересчёт, zone calculation.
- `Services/RuleEvaluation.swift` — 7 примитивов вычислений.
...
Новые опциональные поля в JSON-ответе LLM:
...
- Экран детали: график за 7 дней (значение по дням, линии порогов и warning-зоны), список нарушений с раскрытием до ингредиента.
- Тап по нарушению → переключение таба на Дашборд + установка даты + открытие MealEventEditorView.
- MealSlotCard: цветной левый бордер (жёлтый/красный) + бейдж с количеством нарушений.
- MealEventEditorView: иконка у ингредиента-нарушителя.
- Минимум данных: 3 дня из 7 с ≥1 structured. Меньше → серый индикатор «Недостаточно данных».
...
- `rule-analytics`: агрегирование 7-дневных метрик, 3-зонная оценка, minimum data threshold.
...
- Domain/Services: RuleEngine, RuleEvaluation, 7 примитивов, инкрементальный пересчёт.
- Data model: +2 сущности, +5 полей в EstimateItem.
- Existing flows: MealSlotCard и MealEventEditorView получают индикаторы нарушений.
- LLM: расширенный промпт и контракт ответа.
- Тестирование: unit-тесты для 7 примитивов, evaluation engine, zone calculation.
